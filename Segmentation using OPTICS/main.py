...
def optics(data, eps=np.inf, min_sample=250, min_cluster_size=10, metric='minkowski'):
    print(" Training - ", eps, "_", min_sample, " Begin")
    # optics = OPTICS( min_samples=min_sample, metric = metric, cluster_method='xi', algorithm='ball_tree').fit(data)
    optics = OPTICS(min_samples=min_sample, xi=eps, min_cluster_size=min_cluster_size).fit(data)
...
def get_optimal_params(img, mask, param_grid):
    ...
    for eps in param_grid['eps']:
        for min_samples in param_grid['min_samples']:
            optics_img = optics(img, eps, min_samples)
            accuracy, slice_ = accuracy_IoU_scorer(optics_img, mask)
    ...
    print(f'max acc = {accuracy_matrix.max()}')
...
# -------------------SET PATHS-----------------
#processed_images_path = '/Users/anastasia/PycharmProjects/pythonProject3/processed_images.txt'
#path = '/Users/anastasia/Desktop/Datasets/Total/BlackPupilEye'

path = '/home/nastya/dataset/test/Total/BlackPupilEye'
processed_images_path = '/home/nastya/python/processed_imagesOPTICS.txt'
# ---------------------------------------------


# -------------------SET RESULT FILE-----------------
# accuracy_metrics_path = '/Users/anastasia/PycharmProjects/pythonProject3/metricsOPTICS.txt'
# image_results_path = '/Users/anastasia/PycharmProjects/pythonProject3/predicted_masks/'

#image_results_path = 'out/imagesOPTICS/'
#accuracy_metrics_path = 'out/metricsOPTICS.txt'
# ---------------------------------------------------
...
param_grid = {'eps': [0.4, 0.3, 0.2, 0.1],
              'min_samples': [450, 500, 550, 600]}
...
for i in range(n):
    ...
    optics_img = optics(img_3f, optimal_eps, optimal_samp)
    accuracy, slice_ = accuracy_IoU_scorer(optics_img, masks[i])

    print(f'\neps={optimal_eps}, min_samp={optimal_samp}, acc = {accuracy}')
    #display([out_blur_img, masks[i], slice_])

    with open('out/imagesWithoutBlur/metricsOPTICS.txt', "a") as f:
        f.write(uniq_img_names[i] + ' ' + str(accuracy) + ' ' + str(optimal_eps) + ' ' + str(optimal_samp)+'\n')
        f.close()

    data_2d = slice_.reshape((-1, slice_.shape[-1]))
    np.savetxt('out/imagesWithoutBlur/imagesOPTICS/' + (uniq_img_names[i]) + '.txt', data_2d, delimiter=',')
